...
All variables need to be stored in the app, if I execute it now it will fail, so let’s create all the required variables (API possible)
    ...
    tfh pushvars -overwrite-all -dry-run false   -env-var "AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID" -env-var "AWS_SESSION_TOKEN=$AWS_SESSION_TOKEN"  -senv-var "AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY"   -var "prefix=jerome" -var "placehodler=placedog.net"
